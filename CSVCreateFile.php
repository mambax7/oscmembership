...
if ($sFormat == "addtocart")
{
	...
	$sSQL = "SELECT per_ID FROM person_per $sGroupTable $sJoinFamTable WHERE 1 = 1 AND person_per.chu_Church_ID=" . $_SESSION['iChurchID'] . " $sWhereExt $sGroupBy";
...
else
{
	...
	if ($sFormat == "rollup")
	{
		$sSQL = "(SELECT *, 0 AS memberCount, per_LastName AS SortMe FROM person_per $sGroupTable $sJoinFamTable WHERE per_fam_ID = 0 AND person_per.chu_Church_ID=" . $_SESSION['iChurchID']. " $sWhereExt)
	...
	{
		$sSQL = "SELECT * FROM person_per $sGroupTable $sJoinFamTable WHERE 1 = 1 AND person_per.chu_Church_ID=" . $_SESSION['iChurchID'] . " $sWhereExt $sGroupBy ORDER BY per_LastName";
	}
	...
	if (!empty($_POST["FamilyName"])) $headerString .= "\"Family Name\",";
	...
	while($aRow = mysql_fetch_array($rsLabelsToWrite))
	{
		...
		if (!($bSkipIncompleteAddr && (strlen($sCity) == 0 || strlen($sState) == 0 || strlen($sZip) == 0 || (strlen($sAddress1) == 0 && strlen($sAddress2) == 0))))
		{
			...
			if (!($bSkipNoEnvelope && (strlen($per_Envelope) == 0)))
			{
				...
				if (isset($_POST["FamilyName"])) $sString .= "\",\"" .$fam_Name;
				...
				{
					$sSQLcustom = "SELECT * FROM person_custom WHERE per_ID = " . $per_ID . " AND chu_Church_ID=" . $_SESSION['iChurchID'];
