...
$result1 = $xoopsDB->query("SELECT name FROM ".$xoopsDB->prefix()."_modules WHERE dirname='churchsplash'");
$result2 = $xoopsDB->query("SELECT id, title, comment, weight FROM ".$xoopsDB->prefix()."_churchsplash WHERE NOT weight='0' ORDER BY weight");
...
while($cc_start = $xoopsDB->fetchArray($result2)) {
		...
		<td><a href='".XOOPS_URL."/modules/churchsplash/index.php?id=".$myts->makeTboxData4Show($cc_start['id'])."'>".$myts->makeTboxData4Show($cc_start['title'])."</a></td>
